...
> 2. **`contains text` currently fails at query-compile time.** Verified by executing several `contains text` queries (with and without match options) through `Container.QueryAsync`: every one throws a `NullReferenceException` from `PhoenixmlDb.XQuery.Analysis.SchemaFeatureChecker.VisitStepExpression`, before the query ever runs. A plain, non-full-text predicate compiles and runs fine, so this is specific to `contains text`. This is a real engine defect, not a documentation problem: it is tracked as [`phoenixmldb-xquery#15`](https://github.com/phoenixmldb/phoenixmldb-xquery/issues/15) and was fixed upstream on 2026-09-11. That fix has **not been released** — the pinned package (`PhoenixmlDb.XQuery` 1.6.9) and every later published version predate it, so no package you can install today includes it. The syntax below is what the grammar defines, but until a package carrying that fix ships, it has not been possible to confirm it *executes* successfully. If you're relying on this feature, verify against your own build first.
