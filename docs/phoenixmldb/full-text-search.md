...
- **XQuery's `contains text` does not use this index.** It still evaluates by scanning, exactly as it did before this index existed — correct, just not accelerated. A later phase is candidate-source acceleration for it, and it has a documented prerequisite: this index matches phrases more strictly than `contains text` itself does, so using it as a candidate source naively would silently drop matches the evaluator would otherwise confirm.
